...
params = {'text.latex.preamble' : [r'\usepackage{times}', r'\usepackage{mathptmx}'],
          # 'legend.fontsize': 10,
...
def load_file(fname: str) -> List[Data]:
...
def best_fit_line(data: List[Data]) -> Tuple[List[Data], float]:
    ...
    slope = np.polyfit(log_x, log_y, deg=1, w=log_x)
    # slope = P.polyfit(log_x, log_y, deg=1)
...
def generate_graphs():
    ...
    integrands = [
        ("$C^0$ Discontinuity (additive)", "c0_disc_add"),
        ("$C^0$ Discontinuity (multiplicative)", "c0_disc_mul"),
        ("$C^1$ Discontinuity (additive)", "c1_disc_add"),
        ("$C^1$ Discontinuity (multiplicative)", "c1_disc_mul"),
        ("Clipped Sphere", "clipped_n_ball"),
        ("Extruded Circle (straight)", "extruded_circ_flat"),
        ("Extruded Circle (slight shear)", "extruded_circ_angled"),
        ("Extruded Circle (strong shear)", "extruded_circ_extreme"),
        ("Sphere", "ndsphere"),
    ]
    samplers = [
        ("Bose-OA$(t=2)$", "bush_s2"),
        ("Bush-OA$(t=3)$", "bush_s3"),
        ("CMJ3D-OA$(t=3)$", "cmj3d"),
        ("Sobol $(0,1,2)$", "sobol_good"),
        ("Sobol $(2,14,15)$", "sobol_bad"),
        ("Sobol $2$D", "pbrt_sobol"),
        ("$N$-Rooks", "NRooks"),
        ("Random", "random"),
    ]
    # matplotlib color codes
    color_codes = ["C0", "C1", "C2", "C3", "C4", "C5", "C6", "C7"]
    ...
    for integrand in integrands:
        ...
        for i, sampler in enumerate(samplers):
            ...
            input_data = load_file(input_folder + integrand[1] + "/" +
                                   sampler[1] + suffix)
            try:
                data, slope = best_fit_line(input_data)
                # best_fit_lines.append((f"$O(N^{{{slope:.3f}}})$ {sampler[0]}", data,
                #                        "--", color_codes[i]))
                plot_data.append((sampler[0] + f", $({slope:.2f})$", input_data, "-", color_codes[i]))
                pbar.update(1)
            except TypeError:
                print(
        ...
        ax = var_multi_plot(
            ...
            title=integrand[0],
            ax=None)
        output_fname = f"{output_folder}less_{integrand[1]}"
        var_multi_plot(
            # data=plot_data[:5],
            data = [plot_data[i] for i in [7,6,5,0,1,2]],
            fname=output_fname,
            title=integrand[0],
            ax=ax)
        pbar.update(1)
